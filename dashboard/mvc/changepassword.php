...
if (isset($_POST['update_password'])) {
    ...
    $stmt = mysqli_prepare($con, "SELECT * FROM sms_admins WHERE admin_id=? LIMIT 1");
    mysqli_stmt_bind_param($stmt, "i", $logged_admin_id);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    $user = mysqli_fetch_assoc($result);
    mysqli_stmt_close($stmt);

    if (!$user || !password_verify($old_password, $user['admin_password'])) {
    ...
    if ($n > 0) {
        ...
        // Use prepared statements to prevent SQL injection
        $stmt = mysqli_prepare($con, "UPDATE sms_admins SET admin_password=? WHERE admin_id=?");
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        mysqli_stmt_bind_param($stmt, "si", $hashed_password, $logged_admin_id);
